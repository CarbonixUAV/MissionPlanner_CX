...
namespace Carbonix
{
    public partial class CorridorPlanForm : Form
    {
        ...
        // Loaded corridor centerline
        private List<PointLatLngAlt> corridorLine;
        ...
        // Tracks which corridorLine indices were added interactively (right-click insert).
            ...
            using (var dlg = new OpenFileDialog())
            {
                ...
                if (pts == null || pts.Count < 2)
                {
                    CustomMessageBox.Show(
                        "No line/polyline geometry found in the file.\n" +
        ...
        private void DrawMap()
        {
            ...
            if (corridorLine == null || corridorLine.Count < 2)
            {
            ...
            layer_corridor.Routes.Add(new GMapRoute(
                corridorLine.Select(p => new PointLatLng(p.Lat, p.Lng)).ToList(),
                "centerline")
            {
        ...
        {
            if (corridorLine == null || corridorLine.Count < 2) return;
            ...
            try
            {
                ...
                (wps, profileSamples, terrAlt) = await System.Threading.Tasks.Task.Run(() =>
                {
                    ...
                    var generated = CorridorPlanner.GenerateMission(capturedLine, capturedP, capturedHome);
                    var profile   = CorridorPlanner.BuildElevationProfile(capturedLine, capturedHome, capturedP);
        ...
        /// can be reapplied after a geometry-triggered regeneration.
        /// </summary>
        ...
        private void ElevProfile_AltitudeChanged(object sender, AltChangeEventArgs e)
        {
            ...
            // e.WaypointIndex is the corridor vertex index (0..M-1).
        ...
        {
            if (elevationPoints == null || corridorLine == null || corridorLine.Count < 2) return;
            ...
            int vA = Math.Min(before.WaypointIndex, corridorLine.Count - 1);
            int vB = Math.Min(after.WaypointIndex,  corridorLine.Count - 1);
            var ptA = corridorLine[vA];
            var ptB = corridorLine[vB];
            double newLat = ptA.Lat + t * (ptB.Lat - ptA.Lat);
            double newLng = ptA.Lng + t * (ptB.Lng - ptA.Lng);

            int insertIdx = Math.Min(vA, vB) + 1;
            corridorLine.Insert(insertIdx, new PointLatLngAlt(newLat, newLng, 0));
            ...
            // Shift WaypointIndex for every sample whose corridor vertex moved up.
            foreach (var s in elevationPoints)
                if (s.WaypointIndex >= insertIdx) s.WaypointIndex++;
            ...
            try
            {
                ...
                wps = await System.Threading.Tasks.Task.Run(() =>
                    CorridorPlanner.GenerateMission(capturedLine, capturedP, homePoint));
            }
            catch (Exception ex)
            {
            ...
            foreach (var wp in generatedWps)
            {
                if (wp.CorridorVertexIndex < 0) continue;
                bool isLoiterWp = wp.Command == MAVLink.MAV_CMD.LOITER_TURNS;
                var matching = elevationPoints.FirstOrDefault(s =>
                    s.WaypointIndex == wp.CorridorVertexIndex &&
        ...
        {
            if (corridorLine == null || elevationPoints == null) return;

            _preserveProfileView = true;

            int vIdx = e.WaypointIndex;
            if (vIdx < 0 || vIdx >= corridorLine.Count) return;
            ...
            int vA = Math.Min(prevS.WaypointIndex, corridorLine.Count - 1);
            int vB = Math.Min(nextS.WaypointIndex, corridorLine.Count - 1);
            double t = ComputeLegFraction(e.NewDistM, prevS, nextS);

            var ptA = corridorLine[vA];
            var ptB = corridorLine[vB];
            corridorLine[vIdx] = new PointLatLngAlt(
            ...
            // drag-adjusted values) to the freshly generated waypoints.
            foreach (var wp in generatedWps)
            {
                if (wp.CorridorVertexIndex < 0) continue;
                bool isLoiterWp = wp.Command == MAVLink.MAV_CMD.LOITER_TURNS;
        ...
        private async void ElevProfile_WaypointRemoveRequested(object sender, WaypointRemoveEventArgs e)
        {
            ...
            if (corridorLine == null || removeIdx < 0 || removeIdx >= corridorLine.Count) return;
            if (corridorLine.Count <= 2) return;

            // Preserve the current profile view so removal doesn't jump the pan/zoom.
            _preserveProfileView = true;

            corridorLine.RemoveAt(removeIdx);
        ...
        private void SyncProfileToMapExtent()
        {
            ...
            // Snap the extent to the corridor-vertex samples that bracket the visible
            // range, so the profile always starts/ends at a clean waypoint rather
            // than mid-leg.
            var vtxDists = elevationPoints
                .Where(s => s.IsLineWaypoint)
                .Select(s => s.DistM)
                .OrderBy(d => d)
                .ToList();
        ...
        private void BUT_accept_Click(object sender, EventArgs e)
        {
            ...
            // The final pass is identified by the highest LineIndex in the result.
            int finalPassLineIdx = generatedWps.Max(w => w.LineIndex);

            int exportedCount = 0;
            foreach (var wp in generatedWps)
            {
                // Insert a DO_LAND_START (0,0,0) before every waypoint on the final pass
                // so the autopilot can identify the final survey leg.
                if (wp.LineIndex == finalPassLineIdx)
                {
            ...
            log.Info($"CorridorPlanForm: appended {exportedCount} waypoints to mission (absolute AMSL frame, final pass marked with DO_LAND_START).");
            this.Close();
        }
    }
}
