...
namespace Carbonix.Tests.Warnings
{
    ...
    public class NamedValueConditionTests
    {
        ...
        public void Evaluate_EqualOp_MatchesExactValue()
        {
            ...
            var cond = Condition.NamedValue("state", CompareOp.EQ, 2.0);
            cond.Store = store;
        ...
        public void Evaluate_LessThanOp()
        {
            ...
            var below = Condition.NamedValue("temp", CompareOp.LT, 100);
            below.Store = store;
            var above = Condition.NamedValue("temp", CompareOp.LT, 25);
            above.Store = store;

            Assert.IsTrue(below.Evaluate(new object(), _state));
            Assert.IsFalse(above.Evaluate(new object(), _state));
        }
